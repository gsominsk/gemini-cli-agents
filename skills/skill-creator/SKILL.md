...
Skills are modular, self-contained packages that extend Gemini CLI's capabilities by providing
specialized knowledge, workflows, and tools. Think of them as "onboarding guides" for specific
domains or tasks—they transform Gemini CLI from a general-purpose agent into a specialized agent
...
Skills must be designed to respect and leverage the project's foundational instructions found in `GEMINI.md`. When a skill is active, it should treat `GEMINI.md` as the primary source of project-wide conventions and the "Constitution" of the workspace.
...
For any multi-step procedure or complex investigation, skills MUST instruct Gemini CLI to use the `update_topic` tool. This provides the user with visibility into the strategic intent and maintains a clear narrative of the task's progress.
...
- **Multi-step processes**: See [references/workflows.md](references/workflows.md) for sequential workflows, conditional logic, and Gemini CLI specific operational cycles.
- **Specific output formats or quality standards**: See [references/output-patterns.md](references/output-patterns.md) for tone, security mandates, and Memory Bank patterns.

These files contain established best practices for effective skill design within the Gemini CLI environment.
